...
def build_exaone_prompt(
    ...
    user_prompt = (
        ...
        "2) 제목은 한 줄 분량(20~25자)으로 간결하게 작성하세요.\n\n"
        "3) 본문은 두 줄~세 줄 분량(80~120자)으로 작성하세요.\n"
        "   본문의 마지막 한 줄은 반드시 CTA 문장으로 구성하세요.\n\n"
        "4) 출력 형식은 다음과 같이 작성하세요:\n"
        "   [제목] 레이블 다음 줄에 제목 내용\n"
        "   [본문] 레이블 다음 줄에 본문 내용\n\n"
        "5) 영어 사용은 금지하며, 한국어로만 작성하세요.\n\n"
        "6) 타겟 페르소나의 상황 컨텍스트와 톤 가이드를 적극 반영하세요.\n\n"
        "7) 표현 수위 가이드:\n"
...
class ExaoneToneCorrector:
    ...
    def __init__(self, model_name: str = "LGAI-EXAONE/EXAONE-4.0-1.2B", use_cache: bool = True):
        ...
        if cached:
            ...
            # Initialize bad_words_ids even when using cache
            self.bad_words_ids = []
